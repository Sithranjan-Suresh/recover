...
function sectionHeader(title) {
  return `<h2 style="font-family: Georgia, serif; font-size: 20px; color: #00A876; border-bottom: 2px solid #00A876; padding-bottom: 8px; margin-top: 32px;">${esc(title)}</h2>`;
}
...
export function generateRecoveryPackHTML(context, outputs, plan) {
  ...
  const sections = Object.keys(RENDERERS)
    .map((agentName) => {
      ...
      return `${sectionHeader(AGENT_LABELS[agentName])}${body}`;
    })
    .join('');

  const planHtml = plan
    ? `
      ${sectionHeader('Your Recovery Plan')}
      <p style="color: #888;">Synthesized from all 5 agents</p>
...
<body style="font-family: Helvetica, Arial, sans-serif; color: #111; background: #fff; max-width: 720px; margin: 0 auto; padding: 40px 24px; line-height: 1.6;">
  <div style="border-bottom: 3px solid #00A876; padding-bottom: 16px; margin-bottom: 8px;">
    <h1 style="font-family: Georgia, serif; margin: 0;">Recover</h1>
    <p style="color: #555; margin: 4px 0 0;">Recovery Pack for ${esc(context.jobTitle)}${context.inferredCompany ? ` — formerly at ${esc(context.inferredCompany)}` : ''}</p>
  </div>
  ${sections}
  ${planHtml}
