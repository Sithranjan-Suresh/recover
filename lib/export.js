...
const SECTION_COLORS = {
  linkedinAgent: '#0A66C2',
  outreachAgent: '#00A876',
  unemploymentAgent: '#8957E5',
  jobAlertsAgent: '#E8862C',
  resumeAgent: '#D6336C',
};
...
export function generateRecoveryPackHTML(context, outputs, plan) {
  ...
  const planHtml = plan
    ? `
      ${sectionHeader('Your Recovery Plan', '#111')}
      <p style="color: #888; margin-top: -4px;">Synthesized from all 5 agents</p>
...
<body style="font-family: Helvetica, Arial, sans-serif; color: #111; background: #fff; max-width: 720px; margin: 0 auto; padding: 0 0 40px; line-height: 1.6;">
  <div style="background: linear-gradient(135deg, #0A0D1A, #111425); padding: 36px 32px; margin-bottom: 8px;">
    <div style="display: flex; align-items: center; gap: 10px;">
      <span style="display: inline-block; width: 14px; height: 14px; border-radius: 4px; background: #00D4FF;"></span>
      <span style="font-family: Georgia, serif; font-size: 26px; font-weight: bold; color: #EEF2FF; letter-spacing: -0.5px;">Recover</span>
    </div>
    <p style="color: #9aa3c4; margin: 10px 0 0; font-size: 14px;">Recovery Pack for ${esc(context.jobTitle)}${context.inferredCompany ? ` — formerly at ${esc(context.inferredCompany)}` : ''}</p>
  </div>
